    ...
    for (int id = 0; id < N; id++)
    {
        ...
        for (int i = 0; i < K; i++)
        {
            if (delta_K && rand() % ((K - i) / delta_K) == 0) //если дельта_к и рандом от 0 до (отношение декремента кол-ва команд на
            {
                ...
                {
                    //и рандом от 0 до отношения дельты_к на разность дельта_к и while (true)
                    //дельта_к_блок. С каждым шагом уменьшаем кол-во команд к
                    {
    ...
    for (int i = 0; i < K; i++)
    {
        for (int id = 0; id < N; id++)
        {
            if (data[id][i])
            {
                if (flag == 1)
                {
                    cout << "Data transfer request" << "\t" << data[id][i] <<
                        "\t\tProc " << id + 1 << endl;
                }
                while (true)
                {
            ...
            else
            {
                ...
                if (flag == 1)
                {
                    cout << "Without memory request" << "\t-" << "\t\tProc "
                        << id + 1 << "\t" << endl;
                }
            }
        }
